...
function connectButtonHandler(event) {
    ...
    if (!connected) {
        ...
        button.innerHTML = 'Đang kết nối...';
        connect(username).then(() => {
            button.innerHTML = 'Rời phòng';
            button.disabled = false;
            shareScreen.disabled = false;
        }).catch(() => {
            alert('Connection failed. Is the backend running?');
            button.innerHTML = 'Tham gia thi';
            button.disabled = false;
        });
    }
    else {
        ...
        button.innerHTML = 'Tham gia thi';
        connected = false;
        shareScreen.innerHTML = 'Chia sẻ màn hình';
...
function updateParticipantCount() {
    if (!connected)
        count.innerHTML = 'Không có kết nối.';
    else
        count.innerHTML = (room.participants.size + 1) + ' thí sinh online.';
};
